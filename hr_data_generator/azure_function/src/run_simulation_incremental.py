...
def run_incremental_simulation(engine, sector, seed):
    ...
    # Falls back to the stored state if the loop runs zero iterations (e.g. a
    # same-day re-trigger where nothing new is due yet), so the state is
    # written back unchanged instead of raising on an unset variable.
